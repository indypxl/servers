...
void execution( int internet_socket, struct sockaddr * internet_address, socklen_t internet_address_length )
{
	//Step 2.1
	char packet[100];
	printf("hoeveel packeten wilt u sturen\n");  //<-- slaat packet op
	scanf("%s",packet);
	int packet_number = atoi(packet);
	int number_of_bytes_send = 0;
	number_of_bytes_send = sendto( internet_socket, packet, strlen(packet), 0, internet_address, internet_address_length );
	...
	printf("%d packets are received from the %d needed",timessend,packet_number);
}
